...
describe("ObjectCard", () => {
  it("renders card with object name", () => {
    const card = makeCard();
    render(<ObjectCard card={card} errors={[]} linkNameOnly={true} showObjectName={false} onChange={vi.fn()} onRemove={vi.fn()} />);
  ...
  it("calls onRemove when delete is clicked", () => {
    ...
    render(<ObjectCard card={makeCard()} errors={[]} linkNameOnly={true} showObjectName={false} onChange={vi.fn()} onRemove={onRemove} />);
  ...
  it("calls onChange when object name input changes", () => {
    ...
    render(<ObjectCard card={makeCard()} errors={[]} linkNameOnly={true} showObjectName={false} onChange={onChange} onRemove={vi.fn()} />);
  ...
  it("applies error class on objectName error", () => {
    ...
    render(<ObjectCard card={card} errors={errors} linkNameOnly={true} showObjectName={false} onChange={vi.fn()} onRemove={vi.fn()} />);
  ...
  it("toggles showLabel via toggle switch", () => {
    ...
    render(<ObjectCard card={card} errors={[]} linkNameOnly={true} showObjectName={false} onChange={onChange} onRemove={vi.fn()} />);
  ...
  it("shows object name prefix in simple mode when showObjectName=true", () => {
    const card = makeCard({ showLabel: true, fieldLabel: "商品コード" });
    render(<ObjectCard card={card} errors={[]} linkNameOnly={true} showObjectName={true} onChange={vi.fn()} onRemove={vi.fn()} />);
    expect(document.querySelector(".preview-text")?.textContent).toBe("商品: レコード名(商品コード:値)");
  });
